...
class Less_Cache{
	...
	public static function Get( $less_files, $parser_options = array(), $use_cache = true ){
		...
		if( $use_cache === true ){
	 		...
	 		if( file_exists($list_file) ){
				...
				$list = explode("\n", NRGnetwork_Std::fs_get_contents($list_file) );
		...
		//save the file list
		$cache = implode("\n",$less_files);
		NRGnetwork_Std::fs_put_contents($list_file, $cache);


		//save the css
		$compiled_name = self::CompiledName( $less_files );
		NRGnetwork_Std::fs_put_contents( Less_Cache::$cache_dir.$compiled_name, $compiled );
	...
	public static function Cache( &$less_files, $parser_options = array() ){
		...
		$file = get_template_directory() . "/framework/classes/lib/Less/Less.php";
