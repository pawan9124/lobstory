...
class NRGnetwork_ImportData{
    ...
    public function admin_menu(){
        $demo_page = add_theme_page(
            'Import Demo Data', 
            'Import Demo Data', 
            'manage_options', 
            'nrgnetwork-demo-importer', 
            array($this, 'demo_import_page') 
        );
    }

    public function admin_scripts(){
        if( isset($_GET['page']) && $_GET['page']=='nrgnetwork-demo-importer' ){
            wp_enqueue_style('demo-importer', NRGNETWORK_PLUGIN_URL.'/framework/admin-assets/demo.import.css');
            wp_enqueue_script('demo-importer', NRGNETWORK_PLUGIN_URL.'/framework/admin-assets/demo.import.js', array( 'jquery' ));
        }
    }

    public function theme_after_switch_hook(){
        $controls = nrgnetwork_get_customizer_controls();
        foreach ($controls as $control) {
            if( !get_theme_mod($control['id']) )
                set_theme_mod( $control['id'], isset($control['default']) ? $control['default'] : '' );
        }
        NRGnetwork_Less::create_css_active_theme();
        exit;
    }

    public function after_switch_theme($old_theme_name, $old_theme = false){
        update_option('nrgnetwork_admin_notice', '1');
    }
        ...
        if( isset($_GET['activated']) && $_GET['activated']=='true' ){
            print '<script type="text/javascript">
                    document.addEventListener("DOMContentLoaded", function() {
                        ...
                        iframe_hook.setAttribute("src", "'.admin_url().'/admin-ajax.php?action=nrgnetwork_template_init"); 
    ...
    public function theme_activation_admin_notice(){
        if( get_option('nrgnetwork_admin_notice')!==false && get_option('nrgnetwork_admin_notice')=="0" )
            return;

        print '<div class="updated" id="theme-admin-notice">
                <h3 style="text-transform: uppercase;">Welcome to '.wp_get_theme()->template.' theme.</h3>
                <p>
                    <a href="'.admin_url().'themes.php?page=nrgnetwork-demo-importer" class="button-primary" style="text-decoration:none;"><i class="fa fa-folder-open"></i> One Click Demo Data</a>
    ...
    public function template_hide_admin_notice_hook(){
        update_option('nrgnetwork_admin_notice', '0');
        exit;
    }
    ...
    public function demo_import_page(){
        ...
        $templates = '';
        $file_dir = NRGnetwork_Std::file_require( NRGNETWORK_PLUGIN_DIR.'demo-data/data/' );
        foreach( glob( $file_dir . '*.xml' ) as $filename ) {
            ...
            $img = NRGnetwork_Std::file_require( trailingslashit(NRGNETWORK_PLUGIN_URL).'demo-data/thumbs/'.$img.'.png', true );
            $templates .= '<label>
        ...
        print '<div class="wrap"><div id="icon-tools" class="icon32"></div>';
            print "<h2>Demo Data Importer</h2>
                  $msg";
            print '<form method="post">
                ...
                <tr>
                    ...
                    <td>
                        <span style="color:red;">Please install required plugins such as Envato Toolkit, BuddyPress, Visual Composer, Revolution Slider, Plugin for NRGnetwork Themes, WooCommerce, MailChimp for WordPress and Contact Form 7 before add demo data.</span>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th scope="row"><button type="submit" class="button-primary import-submit">Import Demo Data</button><img class="import-spinner" src="'.NRGNETWORK_PLUGIN_URL.'/img/loader-light.gif" alt="spinner"></th>
                    <td></td>
                </tr>
                </table>
            </form>';
    ...
    public function import_data($param){
        if( !class_exists('WP_Import') )
            require_once( NRGNETWORK_PLUGIN_DIR . "/framework/classes/lib/wp.importer/wp.importer.php" );

        $download_images = isset($param['download_attachment']) && $param['download_attachment']=='1' ? true : false;

        /* File Paths */
        $options_file = "/demo-data/customizer.txt";
        $nav_file = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_URL.'demo-data/navigation.xml', true);
        $nav_file_path = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_DIR.'demo-data/navigation.xml');
        $widget_file = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_URL.'demo-data/widgets.txt', true);
        $widget_file_path = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_DIR.'demo-data/widgets.txt');

        if( isset($param['template']) && !empty($param['template']) ){
            /* Import Selected Content */
            $data_file = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_DIR.'demo-data/data/'.$param['template'].'.xml');
            ...
            if( isset($param['import_theme_options']) && $param['import_theme_options']=='1' ) {
                if (file_exists(NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_DIR . $options_file))) {
                    $options_uri = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_URL . $options_file, true);
                    $option_data = wp_remote_get($options_uri);
                    $op_data = array_key_exists('body', $option_data) ? $option_data['body'] : '';
                    $customizer = new NRGnetwork_Theme_Customizer();
                    if (!empty($op_data) ) {
            ...
            try{
                $main_file = NRGnetwork_Std::file_require(NRGNETWORK_PLUGIN_DIR.'demo-data/main-content.xml');
            ...
            $sub_menu = get_term_by('name', 'Sub-Footer Menu', 'nav_menu');

            set_theme_mod( 'nav_menu_locations', array(
                    'primary' => $primary_menu->term_id,
                    'sub_footer'=> $sub_menu->term_id
                )
            );

            // Set Home Page
            $page_on_front=get_page_by_title( 'Responsive Social Network Theme' );
            $page_for_posts=get_page_by_title( 'Blog 3' );
    ...
    public function import_revo(){
        if( class_exists('UniteFunctionsRev') ){
            global $wpdb;
            $revo_directory = NRGNETWORK_PLUGIN_DIR.'demo-data/revsliders/';
            $revo_files = array();
            $db = new UniteDBRev();
            ...
            foreach( glob( $revo_directory . '*.txt' ) as $filename ) {
                $filename = basename($filename);
                $revo_files[] = NRGNETWORK_PLUGIN_URL.'demo-data/revsliders/'.$filename;
            }
