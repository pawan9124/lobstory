...
require_once( get_template_directory() . "/framework/classes/lib/Less/Cache.php");
...
class Less_Parser{
	...
	private function GetRules( $file_path ){
		...
		if( $cache_file && file_exists($cache_file) ){
			switch(Less_Parser::$options['cache_method']){
				...
				case 'serialize':
					$cache = unserialize(NRGnetwork_Std::fs_get_contents($cache_file));
					if( $cache ){
						touch($cache_file);
						$this->UnsetInput();
						return $cache;
		...
		if( $cache_file ){
			...
			switch(Less_Parser::$options['cache_method']){
				case 'serialize':
					NRGnetwork_Std::fs_put_contents( $cache_file, serialize($rules) );
				break;
				case 'php':
					NRGnetwork_Std::fs_put_contents( $cache_file, '<?php return '.self::ArgString($rules).'; ?>' );
				break;
				case 'var_export':
					//Requires __set_state()
					NRGnetwork_Std::fs_put_contents( $cache_file, '<?php return '.var_export($rules,true).'; ?>' );
				break;
			}
	...
	public function SetInput( $file_path ){
		...
		if( $file_path ){
			$this->input = NRGnetwork_Std::fs_get_contents( $file_path );
		}
