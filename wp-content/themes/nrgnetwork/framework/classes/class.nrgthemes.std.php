<?php
global $nrgnetwork_google_fonts;
class NRGnetwork_ThemesStd{
    ...
    public static function get_mod($name, $default = false){
        ...
        if( !$mod_value )
            return '';
        return !empty($mod_value) ? NRGnetwork_ThemesStd::clear_xss($mod_value) : '';
    }
    ...
    public static function get_option_bg_value($value){
        $bg = NRGnetwork_ThemesStd::get_option_bg(NRGnetwork_ThemesStd::get_mod($value));
        $result = NRGnetwork_ThemesStd::extract_bg_values($bg);
        return $result;
    }

    public static function get_meta_bg_value($value){
        $val = NRGnetwork_ThemesStd::getmeta($value);
        $val = str_replace("$", "|", $val);
        $bg = NRGnetwork_ThemesStd::get_option_bg($val);
        $result = NRGnetwork_ThemesStd::extract_bg_values($bg);
        return $result;
    }
    ...
    public static function google_fonts(){
        global $nrgnetwork_google_fonts;
        if( !empty($nrgnetwork_google_fonts) )
            return $nrgnetwork_google_fonts;
        
        include_once get_template_directory()."/framework/functions/google-fonts.php";
        $nrgnetwork_google_fonts = get_google_webfonts();
    ...
    public static function fs_get_contents($path){
        ...
        if( !empty( $host ) ){
            ...
            $file_content = $wp_filesystem->get_contents( NRGnetwork_Std::file_require($path) );
            return $file_content;
        }
    ...
    public static function fs_get_contents_array($path){
        $file_content = NRGnetwork_ThemesStd::fs_get_contents($path);
        return explode("\n", $file_content );
    }
...
class NRGnetwork_Std extends NRGnetwork_ThemesStd { }