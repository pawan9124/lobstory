...
global $nrgnetwork_sliders;
$nrgnetwork_sliders = array("none" => 'No slider');
$nrgnetwork_sliders = array_merge($nrgnetwork_sliders, NRGnetwork_Std::get_sliders('layerslider'));
$nrgnetwork_sliders = array_merge($nrgnetwork_sliders, NRGnetwork_Std::get_sliders('revslider'));
$nrgnetwork_sliders = array_merge($nrgnetwork_sliders, NRGnetwork_Std::get_sliders('masterslider'));

require_once get_template_directory() . "/framework/classes/class.render.meta.fields.php";

class NRGnetwork_RenderMeta{
    public $items;
    ...
    public function render_meta_section($post, $metabox){
        ...
        foreach ($metabox['args'] as $value) {
            ...
            if ($value['type'] == 'start_group') {
                ...
                print '<div id="'.$value['name'].'" class="'.$value['name'].'" '.$style.'>';
            } elseif ($value['type'] == 'end_group') {
                print '</div><!-- #'.$value['name'].' -->';
            } else {
                $data_dep = "";
                ...
                print "<div id='option_wrapper_" . $value['name'] . "' class='page_option_fieldset' $data_dep>";
                print '<div class="meta-option-field">';
                    if( isset($value['label']) && $value['label'] != '' && !in_array($value['type'], array('checkbox1', 'colorpicker1', 'number1')) ){
                        print "<div><label for='" . $value['name'] . "'>" . $value['label'] . "</label>";
                            if(isset($value['desc']) && !empty($value['desc'])) {
                                print '<div class="field_description" style="max-width:90%">'.$value['desc'].'</div>';
                            }
                        print "</div>";
                    }
                    print "<div>";
                        print "<div class='page_option_field'>".NRGnetwork_RenderMetaFields::getField($value)."</div>";
                    print "</div>";
                print '</div>';
