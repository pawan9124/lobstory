<?php
class NRGnetwork_Less{
    private $directory = "nrgnetwork";
    ...
    public function enqueue_scripts(){
        ...
        wp_enqueue_style( 'nrgnetwork-stylesheet', $css_file );
    }

    // static methods
    public static function create_css_active_theme(){
        $instanse = new NRGnetwork_Less();
        $instanse->create_css();
    }
    ...
    public function create_css(){
        ...
        if( $dir_path!==false ){
            ...
            if( is_user_logged_in() && !file_exists($css_file) ){
                ...
                NRGnetwork_Std::fs_put_contents($css_file, $css_content);
            }
    ...
    public function get_variables(){
        ...
        $variables = NRGnetwork_Std::fs_get_contents_array(get_template_directory() . "/less/variables.less");
    ...
    public function generate_css( $modify_vars=false ){
        ...
        try{
            ...
            $parser->parseFile( NRGnetwork_Std::file_require($theme_dir . 'less/style.less'), trailingslashit(get_template_directory_uri()) );

            if($modify_vars){
                $modified_vars = array();
                $variables = $this->get_variables();
                foreach ($variables as $key => $value) {
                    $mod_value = NRGnetwork_Std::get_mod($key);
                ...
                foreach ($controls as $control) {
                    if( isset($control['type']) && $control['type']=='font_set' ){
                        $v = NRGnetwork_Std::get_mod( $control['id'], $control['default'] );
                        $vals = NRGnetwork_Std::get_option_array($v);
