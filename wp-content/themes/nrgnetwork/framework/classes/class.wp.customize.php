...
if( !function_exists('nrgnetwork_getLessValue') ){
    function nrgnetwork_getLessValue($name){
        global $less_variables;
        if( empty($less_variables) ){
            $less_variables = is_user_logged_in() ? NRGnetwork_Less::get_less_variables() : array();
        }
...
function nrgnetwork_get_customizer_options(){
    global $customizer_options;
    if( !empty($customizer_options) )
        return $customizer_options;
    
    $customizer_options = nrgnetwork_customizer_options();
    return $customizer_options;
}

function nrgnetwork_get_customizer_controls(){
    global $customizer_options;
    if( empty($customizer_options) )
        nrgnetwork_get_customizer_options();
...
function nrgnetwork_sanitize_js_clean( $value ) {
    $value = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', "", $value);
    return $value;
}

class NRGnetwork_Theme_Customizer{
    private $items;
    ...
    function __construct(){
        $this->items = nrgnetwork_get_customizer_options();
    ...
    public function enqueue_scripts(){
        ...
        foreach ($items as $control) {
            if( isset($control['type']) && ($control['type']=='font_set' || $control['type']=='font') ){
                $values = array();
                $option_val = NRGnetwork_Std::get_mod( $control['id'], $control['default'] );

                if( $control['type']=='font_set' )
                    $values = NRGnetwork_Std::get_option_array($option_val);
                else
        ...
        if( !empty($webfonts) ){
            ...
            $fonts_url='';
            if ( 'off' !== _x( 'on', 'Google font: on or off', 'nrgnetwork' ) ) {
    ...
    public function generate_setting($wp_customize, $control, $section_id=false){
        $wp_customize->add_setting($control['id'], array(
            ...
            'capability' => 'edit_theme_options',
            'sanitize_callback' => 'nrgnetwork_sanitize_js_clean'
        ));
            ...
            switch( $control['type'] ){
                case "image":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Image_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "bg_image":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Bg_Image_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "switch":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Switch_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "pixel":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Number_Pixel_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "sub_title":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Sub_Title_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "font_set":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Font_Set_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "font":
                    $wp_customize->add_control(
                        new NRGnetwork_Customizer_Font_Control( $wp_customize, $control['id'],
                           array(
                               'label'      => $control['label'],
                               'section'    => $section_id,
                               'settings'   => $control['id'],
                ...
                case "color":
                    ...
                    break;
                case "backup":
                    $wp_customize->add_control(new NRGnetwork_Customizer_Backup_Control($wp_customize, $control['id'], array(
                         'label' => $control['label'],
                         'section' => $section_id,
                         'settings' => $control['id'],
                         'priority' => $setting_index,
                    ...
                    break;
                case "import":
                    $wp_customize->add_control(new NRGnetwork_Customizer_Import_Control($wp_customize, $control['id'], array(
                         'label' => $control['label'],
                         'section' => $section_id,
                         'settings' => $control['id'],
                         'priority' => $setting_index,
    ...
    public function wp_customizer_import_handler($data){
        if( !empty($data) ){
                ...
                if( !empty($options) and isset($options['mods']) and is_array($options['mods']) ){
                    foreach ($options['mods'] as $key => $value) {
                        set_theme_mod( $key, $value );
                    }
                    NRGnetwork_Less::build_css();
                    return true;
                }
            }
        return false;
    ...
    public function generate_less(){
        if( isset($_POST['wp_customize']) && $_POST['wp_customize']=="on" ){
            $compiled_css = NRGnetwork_Less::get_compiled_css();
            print '<style type="text/css" id="customize_preview_css">'. $compiled_css .'</style>';
        }
    }
