<?php
    ...
    $title_attr = '';
    // background image
    $img = NRGnetwork_Std::get_option_bg_value('page_title_image');
    if( !empty($img) ){
        $title_attr = $img;
    }
    // Background Image
...
<div class="detail-wrr">
    <div class="clip">
        <div class="bg bg-bg-chrome act center-bg" style="<?php print esc_attr($title_attr); ?>"></div>
    </div>
    <div class="layer"></div>
    ...
    <?php if(is_single()): ?>
        <div class="descr-block slide">
          <div class="descr-block-w">
            <h5><span class="tt"><?php esc_html_e('Date', 'nrgnetwork'); ?></span><span>/</span><?php print get_the_date(false, $post); ?></h5>
            <h5><span class="tt"><?php esc_html_e('Category', 'nrgnetwork'); ?></span><span>/</span>
                <?php
                    if( is_singular('portfolio') ){
                        ...
                        foreach ($terms as $term){
                            $cat_title = $term->name;
                            $cat_slug = $term->slug;
                            print "<a href='javascript:;'>$cat_title</a> ";
                        }
                    } else {
