<?php
class NRGnetwork_TPL{
    public static function post_thumbnail(){
        global $post;
        if ( post_password_required() ){
            return;
        }
        $media = NRGnetwork_TPL::get_post_media();
        if( !empty($media) )
            print "<div class='card-image'>$media</div>";
    }
        ...
        if ( is_a( $next_post , 'WP_Post' ) ) {
            ...
            return '<div class="col-xs-12 col-sm-6">
                        <div class="be-post style-2 nav-post">
                            ...
                            (empty($img)?'':'<a href="'.esc_url(get_permalink( $next_post->ID )).'" class="be-img-block">
                                                            <img src="'.$img.'" alt="omg">
                                                            </a>')
        ...
        if ( is_a( $prev_post , 'WP_Post' ) ) {
            ...
            return '<div class="col-xs-12 col-sm-6">
                        <div class="be-post style-2 nav-post">
                            ...
                            (empty($img)?'':'<a href="'.esc_url(get_permalink( $prev_post->ID )).'" class="be-img-block">
                                            <img src="'.$img.'" alt="omg">
                                            </a>').'
                        </div>
                        </div>';
        ...
        if(isset($args)) {
            ...
            if ($my_query->have_posts()) {
                ...
                while ($my_query->have_posts()) {
                    $my_query->the_post();
                    $img = NRGnetwork_TPL::get_post_media('nrgnetwork-related-thumb');
                    $html .= '<div class="col-xs-12 col-sm-4">
                            <div class="be-post style-2">
                                ...
                                <a href="'.esc_url(get_permalink()).'" class="be-post-title">'.get_the_title().'</a>        
                                    '.$img.'
                                <span>
                                    '.NRGnetwork_TPL::clear_urls(wp_trim_words( wp_strip_all_tags(do_shortcode(get_the_content())), 10 )).'
                                </span>
                            </div>                          
                        </div>';
                }
                if($post->post_type == 'portfolio'){
                    print '<div class="row m0 youMightLike '.$post_type_class.'">
...
function nrgnetwork_getPageSlider($onTop){
    global $post;
    $slider_class = 'fullscreen-section no-padding';
    if (NRGnetwork_Std::getmeta('slider') != '' && NRGnetwork_Std::getmeta('slider') != 'none'):
        print '<div id="tt-slider" class="tt-slider '.$slider_class.'">';
            $slider_name = NRGnetwork_Std::getmeta("slider");
...
// Comment Navigation
if( !function_exists( 'nrgnetwork_theme_comment_nav' ) ){
    function nrgnetwork_theme_comment_nav(){
