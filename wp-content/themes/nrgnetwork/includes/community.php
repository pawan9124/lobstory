<?php
class NRGnetwork_Community_Setings{
    public $network_post_term = 'portfolio_entries';
    public $network_post_tag = 'portfolio_tag';
	...
	function __construct(){
        ...
        // remove message
        add_action('wp_ajax_net_remove_private_msg', array($this, 'net_remove_private_msg_hook'));
        add_action('wp_ajax_nopriv_net_remove_private_msg', array($this, 'net_remove_private_msg_hook'));
        ...
        add_action('wp_enqueue_scripts', array($this, 'print_frontend_styles') );
        ...
        if( function_exists('bp_get_option') && is_admin() ){
            ...
            if( empty($active_components['notifications']) || empty($active_components['friends']) || empty($active_components['messages']) ){
                ...
                require_once( $bp->plugin_dir . '/bp-friends/bp-friends-template.php' );
        ...
        if( $network_post_type=='post' ){
            $this->network_post_term = 'category';
            $this->network_post_tag = 'post_tag';
        } else if( $network_post_type=='product' ){
            $this->network_post_term = 'product_cat';
            $this->network_post_tag = 'product_tag';
        }
	}
    ...
    public function get_comment_author_link( $return, $author, $comment_ID ){
        ...
        if($user != false) {
            $url = NRGnetwork_C::get_author_uri($user->ID);
    ...
    public function network_wp_title( $title ){
        ...
        } else if( isset($wp_query->query_vars['network_page']) && $wp_query->query_vars['network_page']=='user-activation' ){
            $title['title'] = esc_html__('User Activation', 'nrgnetwork');
        }
        return $title;
    }
	...
	public function the_content_filter($content){
		if( isset($GLOBALS['post']->ID) && !empty($GLOBALS['post']->ID) ){
			NRGnetwork_STAT::update_post_views();
		}
		return $content;
	}
    ...
    public function community_templates($template){
        ...
        if( isset($wp_query->query_vars['network_page'], $wp_query->query_vars['user']) && $wp_query->query_vars['network_page']=='netauth' && !empty($wp_query->query_vars['user']) ){
            ...
            if( $user_info!==false ){
                NRGnetwork_STAT::update_profile_views($user_info->ID);
                return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-author.php');
            }
        }

        // edit author profile
        else if( is_user_logged_in() && isset($wp_query->query_vars['network_page'], $wp_query->query_vars['user']) && $wp_query->query_vars['network_page']=='netauth-edit' && abs($wp_query->query_vars['user'])==get_current_user_id() ){
            return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-author-edit.php');
        }

        // edit author collection
        else if( isset($wp_query->query_vars['network_page'], $wp_query->query_vars['user'], $wp_query->query_vars['collect']) && $wp_query->query_vars['network_page']=='netcollect' && abs($wp_query->query_vars['user'])>0 && !empty($wp_query->query_vars['collect']) ){
            return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-author-collect.php');
        }
        ...
        // edit work
            ...
            return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-folio-add.php');
        }

        // statistics
        else if( is_user_logged_in() && isset($wp_query->query_vars['network_page']) && $wp_query->query_vars['network_page']=='netauth-stats' ){
            return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-statistic.php');
        }
        ...
        // messages
            ...
            return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-messages.php');
        }

        // messages
        else if( isset($wp_query->query_vars['network_page'],$wp_query->query_vars['user'],$wp_query->query_vars['token']) && $wp_query->query_vars['network_page']=='user-activation' ){
            return NRGnetwork_Std::file_require(get_template_directory() . '/tpl-user-active.php');
        }
        return $template;
    }
    ...
    public function post_like_action_hook(){
        ...
        if( is_user_logged_in() && !empty($post_id) ){
            NRGnetwork_STAT::update_project_like($post_id);
            print '1';
        }
        exit;
    }
	...
	public function post_like_action_blog_hook(){
        $post_id = $_POST['post_id'];
        print NRGnetwork_C::get_likes($post_id);
        exit;
    }
    ...
    public function get_option_collection($user){
		...
		$colls =  NRGnetwork_C::decode($option);
		return $colls;
    }
    ...
    public function save_collections_option($colls, $user){
    	$coll_name = 'add2collection' . $user;
    	$encoded = NRGnetwork_C::encode($colls);
    ...
    public function net_edit_coll_rename_hook(){
        ...
        print json_encode(array('result'=>'yes'));
        exit;
    }
    ...
    public function net_save_profile_hook(){
        ...
        if( is_array($custom_sections) ){
            ...
            update_user_meta( $user, 'custom_section', NRGnetwork_C::encode($custom_section) );
        }
        // socials
        $cs_socials = array();
        ...
        update_user_meta( $user, 'cs_socials', NRGnetwork_C::encode($cs_socials) );
        print '1';
        exit;
    }
    ...
    public function net_folio_content_filter( $where, &$wp_query ){
        global $wpdb;
        $post_term = $this->network_post_term;
        $post_tag = $this->network_post_tag;
        if ( $search_term = $wp_query->get( 'search_ct_value' ) ) {
            $where .= " AND ( {$wpdb->posts}.post_title LIKE '%". $wpdb->esc_like( $search_term ) ."%' OR
                              {$wpdb->posts}.post_content LIKE '%". $wpdb->esc_like( $search_term ) ."%' OR
                              ( {$wpdb->posts}.ID IN (SELECT object_id FROM {$wpdb->term_relationships}
                                          WHERE term_taxonomy_id IN (SELECT term_taxonomy_id FROM {$wpdb->term_taxonomy}
                                                                                             WHERE taxonomy IN ('$post_term', '$post_tag') AND
                                                                                                   term_id IN (SELECT term_id FROM {$wpdb->terms}
                                                                                                                              WHERE name like '%". $wpdb->esc_like( $search_term ) ."%') ) ) )
                            )";
        }
        return $where;
    }
    ...
    public function net_search_portfolios_hook(){
        ...
        if( isset($_POST['ptype']) && !empty($_POST['ptype']) ){
            $network_post_type = in_array(trim($_POST['ptype']), array('post', 'portfolio', 'product')) ? trim($_POST['ptype']) : $network_post_type;
        }
    ...
    public function get_projects_list($posts_query){
        ...
        while ( $posts_query->have_posts() ) {
            ...
            if( has_post_thumbnail() ){
                ...
                $thumb = '<a href="'.esc_url(get_permalink()).'" class="be-img-block">'.NRGnetwork_C::image_filter($img).'</a>';
            } else {
                $thumb = '<a href="'.esc_url( get_permalink() ).'" class="be-img-block no-image-post"><img src="'.get_template_directory_uri().'/img/placeholder-no-image.png" alt="No image"/></a>';
            }
            $cat_html = array();
            ...
            $result .= '<div class="custom-column-5">
                            <div class="be-post item-media">
                                ...
                                <div class="author-post">
                                    ...
                                    <span>by <a href="'.NRGnetwork_C::get_author_uri($author_id).'">'.get_the_author().'</a></span>
                                </div>
                                <div class="info-block">
                                    <span><i class="fa fa-thumbs-o-up"></i> '.NRGnetwork_C::get_likes().'</span>
                                    <span><i class="fa fa-eye"></i> '.NRGnetwork_C::get_views().'</span>
        ...
        foreach ($user_ids as $user) {
            ...
            while ( $posts_query->have_posts() ) {
                ...
                if( has_post_thumbnail() ){
                    ...
                    $user_posts .= NRGnetwork_C::image_filter($img);
                    $index_folio++;
                }
            }
            wp_reset_postdata();
            ...
            if( is_user_logged_in() ){
                if( get_current_user_id() == $user ){
                    ...
                    $is_following = NRGnetwork_C::net_is_following($user);
            ...
            $result .= '<div class="mix custom-column-5 net-item-people">
                            <div class="be-user-block style-2">
                                <a class="be-ava-user style-2 img-round" href="'.NRGnetwork_C::get_author_uri($user).'">
                                    '.get_avatar($user, 115).'
                                </a>
                                ...
                                <a href="'.NRGnetwork_C::get_author_uri($user).'" class="be-use-name">'.$user_name.'</a>
                                <p class="be-user-info">'.get_user_meta($user, 'cs_city', true).', '.get_user_meta($user, 'cs_country', true).'</p>
                                <div class="info-block">
                                    <span><i class="fa fa-thumbs-o-up"></i> '.NRGnetwork_STAT::get_total_likes($user).'</span>
                                    <span><i class="fa fa-eye"></i> '.NRGnetwork_STAT::get_total_views($user).'</span>
                                </div>
                                '.$follow_btn.'
                                <a class="portfolio-link type-1 clearfix" href="'.NRGnetwork_C::get_author_uri($user).'">
                                    '.$user_posts.'
                                    <div class="color_bg">
    ...
    public function net_frontend_content_hook(){
        ...
        $network_post_type = NRGnetwork_Std::get_mod('network_post_type');
        $network_post_type = !empty($network_post_type) ? $network_post_type : 'portfolio';
        $network_post_status = NRGnetwork_Std::get_mod('network_post_status');
        ...
        // disabling content html filter
        remove_filter('content_save_pre', 'wp_filter_post_kses');
        remove_filter('content_filtered_save_pre', 'wp_filter_post_kses');

        $post_id = wp_insert_post($my_post);

        // enabling content html filter
        ...
        if( $network_post_type=='post' ){
            ...
            if( !empty($tags) ){
                wp_set_post_terms($post_id, $tags, 'portfolio_tag');
            }
        } else if( $network_post_type=='product' ){
        ...
        NRGnetwork_Std::setmeta($post_id, 'folio_about', $desc);
        NRGnetwork_Std::setmeta($post_id, 'extra_brand', $extra['brand']);
        NRGnetwork_Std::setmeta($post_id, 'extra_tools', $extra['tools']);
        NRGnetwork_Std::setmeta($post_id, 'extra_color', $extra['color']);
        NRGnetwork_Std::setmeta($post_id, 'extra_copyright', $extra['copyright']);
        NRGnetwork_Std::setmeta($post_id, 'extra_adult', (!empty($extra['adult']) && $extra['adult']=='adult' ? 'adult' : '') );
        NRGnetwork_Std::setmeta($post_id, 'cs_divider', $custom_styles['divider']);
        NRGnetwork_Std::setmeta($post_id, 'cs_space', $custom_styles['space']);
        NRGnetwork_Std::setmeta($post_id, 'cs_padding', $custom_styles['padding']);
        NRGnetwork_Std::setmeta($post_id, 'cs_color', $custom_styles['color']);
        NRGnetwork_Std::setmeta($post_id, 'cs_bg', $custom_styles['bg']);
        NRGnetwork_Std::setmeta($post_id, 'cs_text', $custom_styles['text']);
    ...
    public function print_frontend_styles(){
        $css = nrgnetwork_com_inline_style();
        if(!empty($css)) {
            wp_add_inline_style('nrgnetwork-style',$css);
        }

        $js=nrgnetwork_com_inline_script();
        if(!empty($js)) {
    ...
    public function net_frontend_remove_hook(){
        ...
        if( is_user_logged_in() && wp_verify_nonce( $nonce, 'net_remove_post' ) && abs($post_id)>0 ){
            $result = 'yes';
            $redirect = NRGnetwork_C::get_author_uri(get_current_user_id());
            wp_delete_post( abs($post_id), true );
        }
        print json_encode( array('result' => $result, 'redirect'=>$redirect) );
        exit;
    }
    ...
    public function net_frontend_register_hook(){
        ...
        if ( !$user_id and email_exists($reg_email) == false ){
            ...
            if( $network_user_activation=='email_activation' ){
                $nonce = wp_create_nonce('network_user_activation');
                $nonce = md5(uniqid($nonce, true));
                update_user_meta( $user_id, 'user_activation', 'pending' );
                update_user_meta( $user_id, 'user_activation_key', $nonce );

                $subject = "Activation Email - $blog_name";

                $active_link = home_url('/') . "?network_page=user-activation&user=$user_id&token=$nonce";
                $content .= "\n\nYour activation link <a href='$active_link'><b>Active Now</b></a>\n";
            }
    ...
    public function net_frontend_message_hook(){
        if( is_user_logged_in() ){
            ...
            if( function_exists('buddypress') ){
                ...
                $reply_thread_id         = messages_new_message( array(
                    'subject' => false,
                    'thread_id' => $thread_id,
                    'sender_id' => $user,
                    'recipients' => array($user_data->ID),
                    'content'    => esc_html__('=Recived your message=', 'nrgnetwork')
                ) );
                if ( ! empty( $reply_thread_id ) ) {
                    print '1';
                } else {
    ...
    public function net_get_pager_folios_hook(){
        if( isset($_POST['type'], $_POST['encrypt_args']) ){
            $args = NRGnetwork_C::decode($_POST['encrypt_args']);
            if( !isset($args['paged']) ){
                $args['paged'] = 1;
            }
            ...
            while ( $posts_query->have_posts() ){
                ...
                if( has_post_thumbnail() ){
                    ...
                    $img = get_template_directory_uri().'/img/placeholder-no-image.png';
                    $thumb = '<a href="'.esc_url(get_permalink()).'" class="be-img-block no-image-post"><img src="'.get_template_directory_uri().'/img/placeholder-no-image.png" alt="No image"/></a>';
                }
                $user = get_post_field( 'post_author', get_the_id() );
                if( $_POST['type']=='projects' ){
                    $cat_html = array();
                    $cats = NRGnetwork_TPL::get_post_cats($post_type, get_the_id());
                    foreach ($cats as $cat) {
                        $cat_html[] = '<a href="'.$cat['link'].'" class="be-post-tag">'.$cat['title'].'</a>';
                    }
                    $edit = '';
                    if( $user == get_current_user_id() ){
                        $edit = '<a href="'.NRGnetwork_C::get_folio_edit_uri(get_the_ID()).'" class="folio-edit-uri">Edit</a>';
                    }
                    $result .= '<div class="col-ml-12 col-xs-6 col-sm-4 netauthor-item">
                                    <div class="be-post">
                                        ...
                                        <div class="author-post">
                                            '.get_avatar( $user, 20 ).'
                                            <span>by <a href="'.NRGnetwork_C::get_author_uri($user).'">'.get_the_author().'</a></span>
                                        </div>
                                        <div class="info-block">
                                            <span><i class="fa fa-thumbs-o-up"></i> '.NRGnetwork_C::get_likes().'</span>
                                            <span><i class="fa fa-eye"></i> '.NRGnetwork_C::get_views().'</span>
                    ...
                    $result .= '<div class="col-ml-12 col-xs-6 col-sm-4 netauthor-item">
                                    <div class="be-post style-4">
                                        ...
                                        <div class="author-post clearfix">
                                            '.get_avatar( $user, 20 ).'
                                            <span>by <a href="'.NRGnetwork_C::get_author_uri($user).'">'.get_the_author().'</a></span>
                                            <span class="like-counter"><i class="fa fa-thumbs-o-up"></i> '.NRGnetwork_C::get_likes().'</span>
                                        </div>
                                    </div>                                  
                                </div>';
                }
        ...
        print json_encode(array('count'=>0, 'result'=>''));
        exit;
    }
    ...
    public function net_get_private_messages_hook(){
        if( isset($_POST['thread_id']) && abs($_POST['thread_id'])>0 ){
            ...
            foreach ($messages as $val):
                if( $val->message!='=Recived your message=' ):
                ?>
                    <div class="be-comment">
                        <div class="be-img-comment">    
                            <a href="<?php print NRGnetwork_C::get_author_uri($val->sender_id); ?>" class="img-round">
                                <?php print get_avatar($val->sender_id, 60); ?>
                            </a>
                        </div>
                        <div class="be-comment-content">
                            <span class="be-comment-name">
                                <a href="<?php print NRGnetwork_C::get_author_uri($val->sender_id); ?>">
                                    <?php print get_the_author_meta('display_name', $val->sender_id); ?>
                                </a>
                            </span>
            ...
            endforeach;
            print '<div class="be-comment"></div>';
        }
        exit;
    }
    ...
    public function net_addremove_friend_hook(){
        ...
        if( is_user_logged_in() ){
            ...
            $friend_followers = NRGnetwork_C::decode($data_followers);
            $following = NRGnetwork_C::decode($data_following);

            // Notification
            $data_followers_info = get_user_meta($fid, 'net_data_followers_info', true);
            $followers_info = NRGnetwork_C::decode($data_followers_info);
            ...
            $data_followers = NRGnetwork_C::encode($friend_followers);
            $data_following = NRGnetwork_C::encode($following);
            update_user_meta( $fid, 'net_data_followers', $data_followers );
            update_user_meta( $user_data->ID, 'net_data_following', $data_following );
            // Notification
            $data_followers_info = NRGnetwork_C::encode($followers_info);
    ...
    public function net_check_notification_hook(){
        if( is_user_logged_in() ){
            ...
            $data_followers_info = NRGnetwork_C::encode($new_info);
...
new NRGnetwork_Community_Setings();
...
class NRGnetwork_C{
    public static function image_filter($img, $url=false, $post_id=false){
        if( !is_user_logged_in() ){
            $meta_val = '';
            if( $post_id!==false ){
                $meta_val = NRGnetwork_Std::getmeta('extra_adult', $post_id);
            } else{
                $meta_val = NRGnetwork_Std::getmeta('extra_adult');
            }
            if( $meta_val=='adult' ){
    ...
    public static function decode($param){
        ...
        try {
            if( !empty($param) ){
                $data = !empty($param) ? unserialize( urldecode(trim($param)) ) : array();
            }
        } catch (Exception $e) {
	...
	public static function get_likes($post_id = null){
        ...
        return NRGnetwork_STAT::get_likes($post_id);
	}
    ...
    public static function get_views($post_id = null){
        ...
        return NRGnetwork_STAT::get_views($post_id);
    }
	...
	public static function get_like_button($post_id = null){
        ...
        if( is_user_logged_in() ){
            ...
            $umeta_data = NRGnetwork_C::decode($umeta);
        ...
        return '<a href="javascript:;" class="be-button-vidget like-button blue-style '.$active_class.'" data-id="'.$post_id.'">
                    ...
                    <span class="liked-prject">'.esc_html__('YOU LIKED THE PROJECT', 'nrgnetwork').'</span>
                </a>';
    }
    ...
    public static function get_liked_posts($user){
        ...
        $result = NRGnetwork_C::decode($user_likes_op);
    ...
    public static function get_slug($slug){
        $slug_value = NRGnetwork_Std::get_mod($slug);
        $slug_value = !empty($slug_value) ? $slug_value :$slug;
        return NRGnetwork_Std::create_slug($slug_value);
    }
    ...
    public static function get_author_uri($user){
        $netauth_username = NRGnetwork_Std::get_mod('netauth_username');
        $slug = NRGnetwork_C::get_slug('netauth');
        $user_slug = $user;
        ...
        return NRGnetwork_C::get_uri($nuri, $uri);
    }
...
if( !function_exists('nrgnetwork_get_friends_slug') ):
function nrgnetwork_get_friends_slug() {
    return apply_filters( 'nrgnetwork_get_friends_slug', '' );
}
endif;

if( !function_exists('nrgnetwork_notifications_toolbar_menu') ):
function nrgnetwork_notifications_toolbar_menu(){
    return;
}
endif;

function nrgnetwork_com_inline_style(){
    global $post;
    $network_post_type = NRGnetwork_Std::get_mod('network_post_type');
    $network_post_type = !empty($network_post_type) ? $network_post_type : 'portfolio';
    if( is_singular($network_post_type) ){
        $cs_divider = NRGnetwork_Std::getmeta('cs_divider');
        $cs_space = NRGnetwork_Std::getmeta('cs_space');
        $cs_padding = NRGnetwork_Std::getmeta('cs_padding');
        $cs_color = NRGnetwork_Std::getmeta('cs_color');
        $cs_bg = NRGnetwork_Std::getmeta('cs_bg');
        $cs_text = NRGnetwork_Std::getmeta('cs_text');
        $block1 = "";
        $block2 = "";
        $block3 = "";
        $block4 = "";
        ...
        $css='';
        $css.= '.tt-frontend-viewport .tt-el-embed + .tt-el-embed{ border-top-width: 1px; '.$block1.'}';
        $css.= '.tt-frontend-viewport{ '.$block2.' }';
        $css.= '.tt-frontend-content-wrapper{ '.$block3.' }';
        $css.= '.tt-frontend-viewport .tt-el-embed-text .entry-text,';
        $css.= '.tt-frontend-content-wrapper h3,';
        $css.= '.tt-frontend-content-wrapper h5,';
        $css.= '.tt-frontend-content-wrapper a{ '.$block4.' }';                    
        return $css;
    }
}

function nrgnetwork_com_inline_script(){
    $addnew_text = NRGnetwork_Std::get_mod('addnew_text');
    $addnew_text = !empty($addnew_text) ? $addnew_text : esc_html__('Add Work', 'nrgnetwork');
    $addnew_status = NRGnetwork_Std::get_mod('addnew_status');
    ...
    return 'var netvars = {
        addnew: "'.NRGnetwork_C::get_folio_edit_uri('0').'",
