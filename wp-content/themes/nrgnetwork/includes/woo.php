...
class NRGnetwork_WooCommerce{
    ...
    public function woo_pagination() {
        global $wp_query;
        print '<div class="row pagination m0">';
            NRGnetwork_TPL::pagination($wp_query);
        print '</div>';
    }
    ...
    public function ttwc_st_before_shop_loop_item_title($param){
        ...
        print "<section>";
            ...
            if( has_post_thumbnail() ){
                ...
                print "<img class='img-responsive' src='$fimage[0]'>";
            }
        print "</section>";
    }
    ...
    public function before_shop_loop_item(){
            ...
            <div class="be-post">
                <?php
                    if( has_post_thumbnail() ){
                        ...
                        print '<a href="'.esc_url( get_permalink() ).'" class="be-img-block">'.$img.'</a>';
                    }
                ?>
                <a href="<?php the_permalink(); ?>" class="be-post-title"><?php the_title(); ?></a>
                <span><?php print wp_kses_post( $product->get_categories(', ') ); ?></span>
                <div class="author-post">
                    <?php print get_avatar( $product->post_author, 20 ); ?>
                    <span>by <a href="<?php print NRGnetwork_C::get_author_uri($product->post_author); ?>"><?php the_author(); ?></a></span>
                </div>
        ...
        <?php
        print '<div class="woo-item-content">';
    }
...
if( class_exists( 'woocommerce' ) )
    new NRGnetwork_WooCommerce();

function nrgnetwork_get_woo_cart_link(){
    if( class_exists( 'woocommerce' ) ){
        global $woocommerce;
