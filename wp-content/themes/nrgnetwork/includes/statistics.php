...
class NRGnetwork_STAT{
	...
	public static function update_post_views(){
		...
		if( is_singular('post') || is_singular($network_post_type) ){
			$views = NRGnetwork_Std::getmeta('post_view', $post_id);
			NRGnetwork_Std::setmeta($post_id, 'post_view', abs($views)+1);
		}
		if( is_singular($network_post_type) ){
			...
			update_user_meta( $post_author, $meta, NRGnetwork_C::encode($meta_data) );
		}
	}
	...
	public static function update_profile_views($profile_id){
		...
		$meta_data[$mday] = $views+1;
		update_user_meta( $profile_id, $meta, NRGnetwork_C::encode($meta_data) );
		if( is_user_logged_in() ){
			$visitor = get_current_user_id();
	...
	public static function update_project_like($project_id){
		...
		// portfolio meta
		$post_likes = abs(NRGnetwork_Std::getmeta('post_likes', $project_id));
		$post_likes_data = NRGnetwork_Std::getmeta('post_likes_data', $project_id);
		$liked_users = NRGnetwork_C::decode($post_likes_data);
		...
		// visitor update
		update_user_meta( $user, 'netproject_likes', NRGnetwork_C::encode($umeta_data) );
		update_user_meta( $user, 'netproject_likes_count', $plikes );

		// portfolio meta update
		NRGnetwork_Std::setmeta($project_id, 'post_likes', $post_likes);
		NRGnetwork_Std::setmeta($project_id, 'post_likes_data', NRGnetwork_C::encode($liked_users));
	...
	public static function get_likes($post_ids){
		$post_likes = abs(NRGnetwork_Std::getmeta('post_likes', $post_ids));
